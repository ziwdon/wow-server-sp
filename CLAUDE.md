...
**Players page: "online" requires `latency > 0`, not just `online=1`.** Both `online_sorted()` (Online now card) and `relative_last_online()` (All characters "Last online" column) gate on `c.online AND c.latency > 0`. `characters.latency` is only written to the DB during `Player::SaveToDB()`, which runs on `PlayerSave.Interval` (default 900 s = 15 min) — so a character can have `online=1, latency=0` for up to 15 minutes after login. During that window the character is absent from Online now and shows a time-based label in All characters instead of "online". This is intentional: `latency > 0` also filters bots (which have `online=1` permanently but no real network session and therefore `latency=0` always).
...
**`PresenceAnnouncer` (`app/services/presence.py`) is the login/logout announcer — per-account `characters.online`, not the Players page's latency rule.** Asyncio task started in `lifespan` on `app.state.presence_announcer`, polling every 15 s (`POLL_INTERVAL_SECONDS`) when `presence.json` has `announce_enabled` (Maintenance page → Player Announcements). AC writes `characters.online=1` synchronously at character login (`CharacterHandler.cpp`) and mod-playerbots switches logout to per-character `CHAR_UPD_CHAR_OFFLINE`, so this flag is immediate; `latency` is only written on `PlayerSave.Interval` (issue #32 tracks moving the Players page to this signal). `PresenceTracker` is per **account**: login = 0 → ≥1 online chars (the first char is the human — alt-bots can only be summoned by a logged-in master, so they never cross the boundary; two chars in one poll → account name), logout = two consecutive absent polls (a quick character swap is silent). `acore_auth.account.online` is **not** usable — alt-bot session teardown runs `UPDATE account SET online=0` for the human's account (`WorldSession.cpp:~188`). The tick skips while `runner.current()` is set (an action may own the console), and `reset()`s (silent reseed) when disabled, when `ac-worldserver` isn't `running`, and whenever its `StartedAt` changes — so Stop/Restart/boots never produce a burst. Console/DB failures are logged and dropped, never retried.
