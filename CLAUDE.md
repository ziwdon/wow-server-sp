...
**`skills/wow-server-sp-gamemaster/`** — the active Game Master skill. Source lives here; copy to `~/.claude/skills/wow-server-sp-gamemaster/` to install globally. Invoke it with the `Skill` tool (or it auto-triggers on any question about installation, GM commands, playerbots, AH bot, individual progression, admin app, or troubleshooting). `SKILL.md` routes each topic to one of 15 reference files under `references/`, covering: install phases, client setup, GM commands, worldserver.conf/AC_* env vars, playerbot commands and raid strategies, AH bot, individual progression tiers, admin app usage, troubleshooting, and SQL queries.
...
| `/opt/stacks/azerothcore/backups/` | Consolidated `azerothcore-backup-<label>-<stamp>.tar.gz` archives from daily cron, manual admin backups, and pre-restore safety backups |
...
**`Errors.log` (`sql.sql` channel) — `Table `graveyard_zone` incomplete: Zone <id> Team <0|1> does not have a linked graveyard`:** an upstream **data gap**, not corruption. AzerothCore ships no `graveyard_zone` link for a few zones a real player essentially never dies in — notably **2037 Quel'thalas** (vanilla sliver, map 0) and **3455 The North Sea** (ocean, map 530). Playerbots roam/teleport/drown there, so it surfaces only after hours of uptime (one line per death event). Benign — `GetClosestGraveyard` (`GameGraveyard.cpp:168`) falls back to the default GY (Westfall/Crossroads) — but it breaks the "0 bytes = clean" invariant. Fix = add a neutral (`Faction=0`) `graveyard_zone` row per zone pointing at the nearest existing graveyard, then `reload graveyard_zone` in the console (no restart). This server has applied it live (1448→2037, 922→3455); it is **not baked into the installer**, so a fresh world import reintroduces the noise. Exact SQL + revert: gamemaster `ref-troubleshooting.md`.
...
**Progression page: internal key `"vanilla"` displays as `"Classic"`.** `EXPANSION_LABELS`, `EXPANSION_ICONS`, `TARGET_STATES`, and all JS objects in `partials/progression_page.html` use `"vanilla"` as the dict key throughout. The icon file is `classic.png`. The UI label is `"Classic"`. Do not rename the internal key — it propagates through DB queries, the service, and the template JS.

**Progression page data is embedded once, not fetched separately.** `api_progression_characters` serialises all character rows to JSON (`rows_json`) and injects them into a `<script type="application/json" id="progression-char-data">` tag in the rendered partial. Client-side JS populates the account dropdown and character list entirely from that blob — there is no second API call for characters. The page has no Refresh button by design; `hx-trigger="load"` fires once on page load only.

**Progression apply writes to `acore_characters.character_queststatus_rewarded`.** mod-individual-progression gates expansion content by checking hidden rewarded quests 66001–66013 (QUEST_BASE + 1 through QUEST_BASE + target_state). The service INSERTs the missing rows inside a `SELECT … FOR UPDATE` transaction. Downgrade is blocked both server-side (raises `ValueError`) and in the UI (icons rendered with `state-downgrade`, `pointer-events: none`).
...
**HTMX vendor files (`htmx.min.js`, `htmx-sse.js`) are not in the repo — they are 51/59-byte placeholders.** The install script fetches the real files from unpkg at install time. The redeploy script uses `rsync --exclude` to preserve them in the build dir. If you add a new `rsync -a --delete` step elsewhere, exclude these two paths or the container will serve broken HTMX and the app silently depends on the browser's stale cache. To restore manually: `curl -sSfL -o "$STACK_DIR/build/app/static/htmx.min.js" "https://unpkg.com/htmx.org@2.0.3/dist/htmx.min.js"` (same for `htmx-sse.js` at `htmx-ext-sse@2.2.2`). The redeploy script has a size guard (≤ 200 bytes = placeholder) that aborts the build if the real files are missing.
...
| `docs/superpowers/specs/2026-05-24-settings-description-and-mobile-design.md` | Design spec for settings description parsing fix and mobile responsive layout — read this before touching `config_index.py` comment parsing or the mobile CSS/JS |
