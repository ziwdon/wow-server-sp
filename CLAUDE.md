...
| `azerothcore-admin/data/` | Admin runtime state (mounted `/admin-data`): `maintenance.json` (scheduler config), `maintenance-log.jsonl` (last 20 runs), `presence.json` (login/logout announcer toggle). Created on first write |
...
**Players page + dashboard "Online" card: "online" = PresenceTracker human, `latency > 0` fallback (issue #32).** One pure rule, `players.online_humans(rows, presence)`, is shared by `collect_players()` (Online now card, headline count, All characters "online" label via `CharRow.online_now`) and `db_stats.count_online()` (dashboard card) so they always agree. Per real account with ≥1 `online=1` character: if the tracker snapshot names a human character that is still `online=1`, that character alone is online — immediately, alt-bots excluded; otherwise (tracker has no memory: admin restarted mid-session or account already online at boot; ambiguous `None`; or the remembered human already logged out) every character with `online AND latency > 0` counts. The fallback exists because `characters.latency` is only written during `Player::SaveToDB()` (`PlayerSave.Interval`, default 900 s) — up to 15 min of lag — and because bots have `online=1` permanently with `latency=0`. Both routes read `app.state.presence_announcer.tracker.snapshot()` via `main._presence_snapshot()` (`None` when no announcer is wired, e.g. tests) and pass it as `presence=`; `players.py`/`db_stats.py` never touch asyncio/app state. Online now renders `Account (Character)` (`account.capitalize()`) to match the announcement format.
...
**`PresenceAnnouncer` (`app/services/presence.py`) tracks per-account `characters.online` presence always; announcing is gated by the toggle.** Asyncio task started in `lifespan` on `app.state.presence_announcer`, polling every 15 s (`POLL_INTERVAL_SECONDS`) whenever `ac-worldserver` is running. `presence.json`'s `announce_enabled` (Maintenance page → Player Announcements) only decides whether transitions are sent over the console; the tracker keeps observing when it is off so the Players page / dashboard card (`tracker.snapshot()`, guarded by a `threading.Lock` — `tick` runs on a worker thread, routes read from the event loop) still name the human character. Enabling announcements never announces sessions already known to the tracker. AC writes `characters.online=1` synchronously at character login (`CharacterHandler.cpp`) and mod-playerbots switches logout to per-character `CHAR_UPD_CHAR_OFFLINE`, so this flag is immediate; `latency` is only written on `PlayerSave.Interval` (the Players page now uses the tracker too — see above). `PresenceTracker` is per **account**: login = 0 → ≥1 online chars (the first char is the human — alt-bots can only be summoned by a logged-in master, so they never cross the boundary; two chars in one poll → account name), logout = two consecutive absent polls (a quick character swap is silent). `acore_auth.account.online` is **not** usable — alt-bot session teardown runs `UPDATE account SET online=0` for the human's account (`WorldSession.cpp:~188`). The tick skips while `runner.current()` is set (an action may own the console), and `reset()`s (silent reseed) when `ac-worldserver` isn't `running` and whenever its `StartedAt` changes — so Stop/Restart/boots never produce a burst. Console/DB failures are logged and dropped, never retried.
